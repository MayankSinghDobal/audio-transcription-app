  ...
  useEffect(() => {
    ...
    class ChineseDragon {
      constructor(yOffset, startDelay, colors) {
        this.segments = [];
        this.length = 15; // Reduced from 20
        this.segmentLength = 20; // Reduced from 25
        this.baseX = -200;
        this.baseY = window.innerHeight * yOffset;
        this.speed = 2; // Reduced from 3
      ...
      update(time) {
        ...
        this.segments[0].y = this.baseY + Math.sin((time + this.timeOffset) * 0.002) * 40; // Reduced from 60
        for (let i = 1; i < this.length; i++) {
          const prev = this.segments[i - 1], curr = this.segments[i];
          curr.x += (prev.x - this.segmentLength - curr.x) * 0.15;
          curr.y += (prev.y + Math.sin((time + this.timeOffset + i * 150) * 0.003) * 15 - curr.y) * 0.15; // Reduced from 20
        }
        if (Math.random() < 0.05) { // Reduced from 0.08
          const head = this.segments[0];
          this.breathParticles.push({ x: head.x + 35, y: head.y, vx: 2 + Math.random(), vy: (Math.random() - 0.5) * 1, life: 1, size: 3 + Math.random() * 2 }); // Reduced size and speed
        }
      ...
      draw(ctx) {
        ctx.save();
        ctx.strokeStyle = this.colors[0]; ctx.lineWidth = 20; ctx.lineCap = 'round'; ctx.lineJoin = 'round'; ctx.shadowColor = this.colors[1]; ctx.shadowBlur = 10; // Reduced from 15
        ...
        for (let i = 0; i < this.length; i++) {
          const seg = this.segments[i], radius = 15 * seg.scale; // Reduced from 18
        ...
        const head = this.segments[0];
        ctx.shadowColor = this.colors[1]; ctx.shadowBlur = 15; ctx.beginPath(); ctx.ellipse(head.x, head.y, 25, 18, 0, 0, Math.PI * 2); ctx.fillStyle = this.colors[0]; ctx.fill(); // Reduced from 28, 20
        ctx.beginPath(); ctx.ellipse(head.x + 25, head.y + 2, 15, 10, 0, 0, Math.PI * 2); ctx.fillStyle = this.colors[0]; ctx.fill(); // Reduced from 18, 12
        ctx.shadowColor = '#FF0000'; ctx.shadowBlur = 6; ctx.beginPath(); ctx.arc(head.x + 8, head.y - 8, 5, 0, Math.PI * 2); ctx.fillStyle = '#FF4500'; ctx.fill(); // Reduced from 6
        ...
        ctx.shadowColor = '#D2691E'; ctx.shadowBlur = 4; ctx.strokeStyle = '#D2691E'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(head.x - 15, head.y - 18); ctx.lineTo(head.x - 25, head.y - 35); ctx.moveTo(head.x - 20, head.y - 25); ctx.lineTo(head.x - 30, head.y - 28); ctx.stroke(); // Reduced from 4
        ctx.shadowColor = '#FFD700'; ctx.shadowBlur = 6; ctx.strokeStyle = '#FFD700'; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(head.x + 35, head.y - 5); ctx.lineTo(head.x + 65, head.y - 2); ctx.moveTo(head.x + 35, head.y + 5); ctx.lineTo(head.x + 65, head.y + 8); ctx.stroke(); // Reduced from 3, 8
        ctx.shadowColor = '#FFFFFF'; ctx.shadowBlur = 12; ctx.beginPath(); ctx.arc(head.x + 8, head.y + 18, 5, 0, Math.PI * 2); ctx.fillStyle = 'rgba(255, 255, 255, 0.9)'; ctx.fill(); // Reduced from 6
        ctx.shadowBlur = 0; ctx.beginPath(); ctx.arc(head.x + 10, head.y + 16, 2, 0, Math.PI * 2); ctx.fillStyle = 'rgba(255, 255, 255, 0.6)'; ctx.fill();
        ctx.shadowColor = 'transparent'; ctx.shadowBlur = 0;
        const legPositions = [3, 12]; // Reduced from [3, 7, 12, 16]
